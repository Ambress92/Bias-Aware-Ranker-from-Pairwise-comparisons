...
class Pairwise_with_rev:
    ...
    def barp_multi_obj(self, params, rev_params):
        """Compute the negative penalized log-likelihood, for the item scores (params) and the reviewers' parameters"""
        val = self._penalty * np.sum(params**2)
        for i in self._data:
            for win, los in self._data[i]:
                val += np.logaddexp(0, -(params[win] + np.dot(self._classes[win,:],rev_params[i,:]) - params[los] - np.dot(self._classes[los,:],rev_params[i,:])))
        return val
    
    def barp_multi_obj_class(self, params, rev_params_class, rev_params, class_index):
        """Compute the negative penalized log-likelihood, for the item scores (params) and the reviewers' parameters"""
        val = self._penalty * np.sum(params**2)
        for i in self._data:
            for win, los in self._data[i]:
                val += np.logaddexp(0, -(params[win] +np.dot(self._classes[win, np.r_[:class_index, class_index+1:self._classes.shape[1]]], rev_params[i,np.r_[:class_index, class_index+1:rev_params.shape[1]]]) + self._classes[win,class_index]*rev_params_class[i] - params[los] - np.dot(self._classes[los, np.r_[:class_index, class_index+1:self._classes.shape[1]]], rev_params[i,np.r_[:class_index, class_index+1:rev_params.shape[1]]]) - self._classes[los,class_index]*rev_params_class[i]))
        return val
    
    def barp_multi_gradient_scores(self, params, rev_params):
        """Compute the gradient of the negative log-likelihood wrt the item scores"""
        grad = 2 * self._penalty * params
        for i in self._data:
            for win, los in self._data[i]:
                z = 1 / (1 + _safe_exp(params[win] + np.dot(self._classes[win,:],rev_params[i,:]) - params[los] - np.dot(self._classes[los,:],rev_params[i,:])))
                grad[win] += -z
                grad[los] += +z
        return grad
    
    def barp_multi_gradient_revs(self, params, rev_params_class, rev_params, class_index):
        """Compute the gradient of the negative log-likelihood wrt the reviewers' parameters"""
        grad = np.zeros(len(rev_params_class))
        for i in self._data:
            for win, los in self._data[i]:
                
                z =  (self._classes[los,class_index] - self._classes[win,class_index])/ (1 + _safe_exp(params[win] + np.dot(self._classes[win, np.r_[:class_index, class_index+1:self._classes.shape[1]]], rev_params[i,np.r_[:class_index, class_index+1:rev_params.shape[1]]]) + self._classes[win,class_index]*rev_params_class[i] - params[los] - np.dot(self._classes[los, np.r_[:class_index, class_index+1:self._classes.shape[1]]], rev_params[i,np.r_[:class_index, class_index+1:rev_params.shape[1]]]) - self._classes[los,class_index]*rev_params_class[i]))
...
def barp_multi_alternate_optim(size, num_reviewers, pc_with_revs, classes, iters = 101, tol = 1e-5, gtol = 1e-5):
    '''x0 is the estimated scores
       y0 is the estimated bias for each reviewer'''
    x0 = np.zeros(size)
    y0 = np.zeros((num_reviewers,classes.shape[1]))


    for i in range(iters):
        
        for class_index in range(classes.shape[1]):
        # minimize with x fixed and update y
            
            res_y = minimize(lambda y: pc_with_revs.barp_multi_obj_class(x0, y, y0, class_index), y0[:,class_index],tol = tol, jac=lambda y: pc_with_revs.barp_multi_gradient_revs(x0, y, y0, class_index), options={"gtol": gtol,'maxiter': 1})
            y0[:,class_index] = res_y.x

        # minimize with y fixed and update x
        res_x = minimize(lambda x: pc_with_revs.barp_multi_obj(x, y0), x0,tol = tol, jac=lambda x: pc_with_revs.barp_multi_gradient_scores(x, y0), options={"gtol": gtol,'maxiter': 1})
        x0 = res_x.x
